...
def create_edges_and_distances(gw, sites, device, threshold, logger):
    ...
    assert gw.crs.to_epsg() == 26990, logger.error("gw crs is not 26990")
    assert sites.crs.to_epsg() == 26990, logger.error("sites crs is not 26990")
...
def classify_pfas_gw(pfas_gw, logger):
    ...
    pfas_gw['sum_PFAS_class'] = pd.cut(pfas_gw['sum_PFAS'], bins=[-1, 0, 10, 10000], labels=[0, 1, 2])
...
def create_hetdata(pfas_gw,unsampled_gw,  pfas_sites, device, pfas_gw_columns, pfas_sites_columns, gw_features, distance_threshold, logger):
    ...
    ## drop ghost duplicates WSSN
    pfas_gw = pfas_gw.drop_duplicates(subset='WSSN')    #### NOTE: THIS MUST BE CHECKED LATER
